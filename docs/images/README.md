...
The v0.12.0-beta.7 Parameters capture shows RK45 h0/AUTO rows, F1 INIT and F2 ADV. Scalar Graph frames have no TIME label; SYS2 uses TIME/PHASE and enabled Event adds EVT.
...
Beta.7 retains the gallery size and regenerates it from the candidate source.
Additional maximum-row, per-IC color and dedicated busy views are produced by
`tools/capture_audit.py` in `docs/ui-review/audit-overview.png`. Busy captures use
host RTC/poll fixtures and an LCD shadow for partial uploads. They are application
renderer output, not device photographs, an SH emulator or physical timing proof.
