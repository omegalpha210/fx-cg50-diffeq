...
The v0.12.0-beta.8 Parameters capture shows RK45 h0/AUTO rows, F1 INIT and F2 ADV. Scalar Graph frames have no TIME label; SYS2 uses TIME/PHASE and enabled Event adds EVT.
...
Beta.8 retains the existing gallery size and regenerates it from candidate source.
The Drawing bar, mixed Output, sparse consumers and numeric EXIT views are in
`docs/ui-review/visibility-overview.png`, generated by `tools/capture_visibility.py`.
All are production C renderer/LCD-adapter frames, not device photographs or a
physical timing proof. Additional historic review generators are retained.
