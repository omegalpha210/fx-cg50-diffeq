...
for keys,expected in [('A:ADD','x'),('ADD','+'),('A:SUB','y'),('SUB','-'),
    ...
    run('F1 4 '+keys,['TEXT 144 57 '+expected+'\n'])
run('F1 4 SIN F6',['Check equation','Syntax error'],['Initial Conditions'])
run('F1 4 SIN F3 EXE',['Check equation','sin('],['View Window'])
run('F1 4 LEFT ACON F2 F2',['sinh('],['Insert function'])
run('F1 4 LEFT ACON F1 F1',['y1'],['Insert variable'])
run('F1 4 NEG A:SUB EXE',['-y'],['Initial Conditions'])
run('F1 1 1 DOWN',['g(y)','TEXT 144 79 y^2-1'])
run('F1 4 LEFT',['DIFF EQ / General 1st','NEXT','V-WIN'],['OUTPUT','SET'])

families=[('F1 1','Separable',"y' = f(x) * g(y)"),
 ('F1 2','Linear 1st',"y' + f(x)*y = g(x)"),
 ('F1 3','Bernoulli',"y' + f(x)*y = g(x)*y^n"),
 ('F1 4','General 1st',"y' = f(x,y)"),
 ('F2','Linear 2nd',"y'' + f(x)*y' + g(x)*y = h(x)"),
 ('F3 9 F6','N-th order','y^(9) = f(x,y,y1,...,y8)'),
 ('F4 9 F6','1st order system',"yi' = fi(x,y1,...,y9), i=1..9")]
...
run('F4 9 F6 '+'DOWN '*8+'0 EXE F6 '+'DOWN '*9,["y9'",'Initial Conditions','y9'])
for invalid in ['0','1 0','NEG 2','1 DOT 5','LEFT ACON']:
    run('F3 '+invalid+' F6',['Invalid dimension'],['DIFF EQ / Higher order'])
run('F3 3 F6 5 EXE EXIT EXE F6',['TEXT 144 57 5\n'],['Change Equation Size'])
run('F3 3 F6 5 EXE EXIT 2 F6 F5 EXIT',['Change Equation Size','Differential Equation'])
kept=run('F3 3 F6 5 EXE EXIT 2 F6 F5 3 F6')
assert 'TEXT 144 57 5\n' in kept[kept.rfind('TEXT 14 9 DIFF EQ /'):]
run('F3 3 F6 OPTN 2 F1',['Convert to system','1st order system','y3'])
run('F1',['OPEN'],['PG-','PG+','OK'])
run('F2 F6 F4 DOWN 2 EXE F5',['IC 2','IC 1','Initial Conditions'],['field 1/','IC 2/2'])
run('F2 F6 NEG 2 EXE',['Initial Conditions','x0','-2','y0'],['Expression editor'])
run('F1 4 F6 F5 F6 F6',['No initial conditions.','TRACE'])
run('F2 F6 F5',['This mode needs at least one IC set.'],['No initial conditions.'])
run('F1 4 F6 F5 F6 DOWN DOWN DOWN DOWN 0 EXE F6',['Cannot calculate'],['TRACE'])
run('F1 4 NEG A:SUB F3 EXIT F6 DOWN 2 F6 F4 EXIT DOWN DOWN 0 DOT 0 5 F5 EXIT',
    ['-y','Initial Conditions','Output selection','Parameter','0.05'])
run('F2 F6 SIN F1 EXE',['Invalid value','sin('])
# Repeated PREV/NEXT retains edited fields and manual parameters exactly.
edited='F1 4 NEG A:SUB F6 DOWN 2 F6 NEG 3 EXE 3 EXE 0 DOT 0 5 EXE'
baseline=run(edited)
...
_,images=run('F2 F6 F6',frames=True)
...
prefix='F1 4 F3 NEG 6 DOT 3 EXE 6 DOT 3 EXE EXIT F6 F6'
...
tail=out[out.rfind('TEXT 14 9 Parameter'):]
assert 'TEXT 144 35 -3\n' in tail and 'TEXT 144 57 3\n' in tail
run('F1 4 F3 0 DOT 1 EXE 0 DOT 2 EXE EXIT F6 F6',['TEXT 144 35 0.1\n','TEXT 144 57 0.2\n'])
run('F1 4 F3 DOWN DOWN DOWN 0 DOT 0 5 EXE',['dot','0.05','12.6'])
run('F1 4 F3 S:EXP EXE',['3.14159265'])
run('F1 4 F3 A:ADD EXE',['Invalid value'])
run('F1 4 F3 NEG 7 EXE EXIT EXIT EXIT F2 F3',['View Window','-7'])
...
run('F1 4 F6 F6 F6 F5 F5',['ICPT: Not available'])
...
run(graph+'F1 DOWN RIGHT',["IC1 x=0 y'=0","IC1 x=0.1 y'=-0.09983333"])
run('F1 4 NEG A:SUB F6 F6 F6 F1 F1 1 EXE',['IC1 x=1 y=0.3678798'])
hidden=run('F2 F6 F6 F4 DOWN DOWN F1 EXIT F6 F1 DOWN',['IC1 x=0 y=1'])
...
run('F4 2 F6 F6 F6 F6 OPTN 1 F2 F3',['TRACE'])
...
run('F2 F6 F6 F4 DOWN F1 F2',['G L','_ L','_ _','RIGHT/F3: color'])
run('F4 2 F6 F6 F6 F6 F4 F2 F3 F5',
    ['Table / IC1 / +x','Finding table end...','Table / IC1 / -x','STAT data saved'])
