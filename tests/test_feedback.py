...
for prefix,menus in [('2 LEFT',['F2']),('1 4 LEFT RIGHT',['F2']),
    ...
    for menu in menus:
        ...
        for key in ['EXE','LEFT','RIGHT','UP','DOWN']:
            assert plot(run(prefix+' '+menu+' '+key))==plot(run(prefix+' '+key)),(prefix,menu,key)
        if menu=='F2' or prefix.startswith(('3','4')):
            assert plot(run(prefix+' '+menu+' F6 F6 LEFT'))==plot(run(prefix+' LEFT'))
blank='1 4 LEFT ACON '
...
assert 'Parameter' in tail(run(output+'DOWN EXE'))
modified=output+'DOWN RIGHT '
